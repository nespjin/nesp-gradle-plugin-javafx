...
final class LayoutInnerResourceClassLoader {
    private static final String LAYOUT_RES_FILE_EXTENSION = ".fxml";

    private LayoutInnerResourceClassLoader() {
    }
    ...
    static void loadFromDirPaths(ClassLoader classLoader,
        ...
        rInnerClass.setName(ResourceType.LAYOUT.name);
        rInnerClass.setjDoc("This is the layout resource");
    ...
    private static void loadFromDirPath(ClassLoader classLoader,
        ...
        for (File file : files) {
            if (file.isFile() && file.getAbsolutePath().endsWith(LAYOUT_RES_FILE_EXTENSION)) {
                String name = file.getName();
                if (name.endsWith(LAYOUT_RES_FILE_EXTENSION)) {
                    name = name.substring(0, name.lastIndexOf(LAYOUT_RES_FILE_EXTENSION));
                }
